...
| `test_cross_thread_send_stop_on_close` | 跨线程持续 send + 对端关闭 + acquire/release | on_connect 中 acquire，工作线程循环 send，服务端 50ms 后关闭连接，on_close 触发后 send 停止（atomic closed 标志），worker release，worker_done==true |
