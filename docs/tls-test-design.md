...
- 共享回调：`_tls_srv_accept_cb`（保存 srv_conn + 设置 userdata）、`_tls_srv_close_cb`（计数+关闭 server）、`_tls_srv_read_echo_cb`（回显）
...
| `test_keylog_disable` | ctx_set_keylog(NULL) | 启用后禁用返回 0，不崩溃 |
...
| `xylem_tls_ctx_set_keylog` | 启用/禁用 | `test_keylog_write`、`test_keylog_disable` |
