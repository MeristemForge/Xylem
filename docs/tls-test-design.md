...
| 测试函数 | 覆盖的功能 | 验证点 |
|---------|-----------|--------|
| `test_cross_thread_send` | 跨线程 xylem_tls_send | 工作线程发送 "hello"，客户端收到回显数据一致，received_len==5 |
| `test_cross_thread_close` | 跨线程 xylem_tls_close | 工作线程调用 close，on_close 回调触发，close_called==1 |
