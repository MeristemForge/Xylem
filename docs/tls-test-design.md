...
`tests/test-tls.c` 包含 22 个测试函数（其中 1 个暂时禁用），覆盖 `src/xylem-tls.c` 的所有公共 API 和 TLS 特有的内部分支。
...
| 测试函数 | 覆盖的功能 | 验证点 |
|---------|-----------|--------|
| `test_sni_hostname` | opts.hostname → SNI 设置 | dial 时传入 hostname，关闭不崩溃 |
...
| 测试函数 | 覆盖的功能 | 验证点 |
|---------|-----------|--------|
| `test_cross_thread_send` | 跨线程 xylem_tls_send + acquire/release | on_connect 中 acquire，工作线程发送 "hello" 后 release，客户端收到回显数据一致，received_len==5 |
| `test_cross_thread_close` | 跨线程 xylem_tls_close + acquire/release | on_connect 中 acquire，工作线程调用 close 后 release，on_close 回调触发，close_called==1 |
| `test_cross_thread_send_stop_on_close` *(temporarily disabled)* | 跨线程持续 send + 对端关闭 + acquire/release | on_connect 中 acquire，工作线程循环 send，服务端 50ms 后关闭连接，on_close 触发后 send 停止（atomic closed 标志），worker release，worker_done==true |
...
| `_tls_tcp_connect_cb` | SNI 路径：opts.hostname 非 NULL 时设置 SNI | `test_sni_hostname` |
| `_tls_tcp_accept_cb` | 正常路径：创建 TLS conn + set_accept_state + do_handshake | `test_handshake_and_echo`, `test_close_server_with_active_conn` |
...
| `_tls_tcp_read_cb` | 其他 SSL_read 错误：xylem_tls_close（尝试 close_notify + 关闭 TCP） | （未覆盖：回环测试中难以触发非 ZERO_RETURN 的 SSL_read 错误）|
| `_tls_tcp_read_cb` | closing 检查：on_read 中触发 close 后退出循环 | `test_handshake_and_echo` |
| `_tls_tcp_close_cb` | 正常路径：SSL_free + on_close + loop_post | `test_handshake_and_echo`, `test_close_server_with_active_conn` |
| `_tls_tcp_close_cb` | server 连接：从链表移除 | `test_handshake_and_echo`, `test_close_server_with_active_conn` |
...
| `xylem_tls_conn_acquire` / `xylem_tls_conn_release` | on_connect 中 acquire 递增引用计数，工作线程完成后 release 递减引用计数 | `test_cross_thread_send`, `test_cross_thread_close`, `test_cross_thread_send_stop_on_close` *(temporarily disabled)* |
| `xylem_tls_close`（跨线程） | 非事件循环线程调用 → `_tls_deferred_close_cb` 转发到事件循环线程执行 | `test_cross_thread_close` |
| `xylem_tls_send`（跨线程 + 连接关闭） | 工作线程持续 send，连接关闭后 atomic closing 检查拒绝发送 | `test_cross_thread_send_stop_on_close` *(temporarily disabled)* |
