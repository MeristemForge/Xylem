...
`xylem_tls_send` 跨线程调用时，递增引用计数（`atomic_fetch_add`），分配 `_tls_deferred_send_t`（包含 TLS conn 指针和数据副本），通过 `xylem_loop_post` 转发到事件循环线程。回调 `_tls_deferred_send_cb` 在执行前检查 `handshake_done` 和 `closing` 状态（连接可能在 post 期间关闭），处理完毕后递减引用计数（`_tls_conn_decref`）。

`xylem_tls_close` 跨线程调用时，递增引用计数后通过 `xylem_loop_post` 将 `_tls_deferred_close_cb` 转发到事件循环线程执行，回调完成后递减引用计数。若 `xylem_loop_post` 失败则立即递减引用计数（`_tls_conn_decref`），避免引用计数泄漏。
