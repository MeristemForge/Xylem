...
本文档定义 Xylem C 工具库中非阻塞 TCP 网络模块的功能需求。该模块基于 `xylem_loop_t` 事件循环，提供 TCP 服务端监听/接受、客户端异步连接、库内部读取与帧解析、缓冲写入、读写超时、心跳检测和自动重连。所有需求均从已批准的设计文档派生，遵循 EARS 模式和 INCOSE 质量规则。
...
- **Handler**: 聚合回调结构体 (`xylem_tcp_handler_t`)，包含用户注册的事件回调函数
