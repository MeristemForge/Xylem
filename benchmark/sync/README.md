...
```
benchmark/sync/
  xylem-sync/main.c      all five primitives in one C binary (-> sync-xylem)
...
| waitgroup | `N` rounds; each spawns `T` tasks that signal done, then waits | `T*N` |
...
- The mapping is "closest idiomatic equivalent", not a byte-identical port:
  - **cond** — Tokio async has no condition variable; the Rust column uses a
    ...
    `sync.Cond`.
  - **waitgroup** — Rust uses `JoinSet` (await N tasks), the idiomatic Tokio
