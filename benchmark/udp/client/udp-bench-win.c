/* ==========================================================================
 * udp-bench-win: UDP load generator (Windows / IOCP).
 *
 * Unlike epoll/kqueue (which report readiness), IOCP reports completion of
 * previously-issued overlapped operations. So instead of "wait for readable
 * then recv", we POST a WSARecv/WSASend and the completion port hands the op
 * back when the kernel has finished it.
 *
 * UDP is connectionless, so there is no "connrate" mode (cf. tcp-bench-win).
 * Each socket is connect()ed to fix the peer address (a local, synchronous
 * operation for UDP -- it only sets the default destination), then associated
 * with the completion port. Datagrams are message-framed: one WSARecv
 * completion == one echoed reply, so there is no stream re-assembly.
 *
 * A single thread drives GetQueuedCompletionStatus in a loop, mirroring the
 * single-threaded client design of the POSIX side (udp-bench-unix.c) -- the
...
typedef enum { OP_RECV, OP_SEND } op_kind_t;
...
static sock_t make_socket(void) {
    return WSASocketW(AF_INET, SOCK_DGRAM, IPPROTO_UDP,
...
static void run_throughput(int argc, char** argv) {
    ...
    for (int i = 0; i < target_conns; i++) {
        ...
        /* connect() on UDP only fixes the peer; it is local + synchronous. */
    ...
    /* prime: post a recv + initial send on each socket */
    for (int i = 0; i < target_conns; i++) {
        conn_t* c = &conns[i];
    ...
    while (g_running && now_us() < deadline) {
        ...
        if (!ok) continue; /* datagram lost or error; socket stays idle */

        if (op->kind == OP_RECV) {
            /* one completion == one echoed datagram */
            uint64_t lat = now_us() - c->send_ts;
            if (lat_count < LATENCY_SLOTS) latencies[lat_count++] = lat;
            msgs_recv++;
            /* ping-pong: fire the next datagram, re-arm recv */
            c->send_ts = now_us();
            if (post_send(c, payload_size)) msgs_sent++;
            post_recv(c);
        }
        /* OP_SEND completions need no action. */
    }
...
static void run_memory(int argc, char** argv) {
    ...
    for (int i = 0; i < target_conns; i++) {
        ...
        CreateIoCompletionPort((HANDLE)c->sock, iocp, (ULONG_PTR)c, 0);
        post_recv(c);
        c->created = true;
        created++;
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: udp-bench-win <mode> [options]\n");
