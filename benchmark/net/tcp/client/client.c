 ...
 *
 * The readiness backends (epoll/kqueue) "wait for writable then write"; the
 * IOCP backend instead POSTs a WSASend/WSARecv/ConnectEx and the completion
 * port hands the op back when the kernel has finished it. Both drive a single
 * client thread -- the *server* is what is being benchmarked, not the client.
 *
 * The platform-neutral parts (timing wrappers, RSS sampling, JSON/statistics
 * emission, argument parsing, dispatch) are shared; only the event backend
 * and the per-mode run_* loops are split behind #ifdef _WIN32.
 * ========================================================================== */
