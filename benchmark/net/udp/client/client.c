/* ==========================================================================
 * udp-bench: cross-platform UDP load generator.
 *
 ...
 *
 * UDP is connectionless, so there is no "connrate" mode (cf. the tcp client).
 * Each client socket is connect()ed to fix the peer address (a local,
 * synchronous operation for UDP -- it only sets the default destination),
 * which lets us use plain read/write (recv/send) and lets the kernel deliver
 * only datagrams from that peer. Datagrams are message-framed: one recv ==
 * one echoed reply, so there is no stream re-assembly (unlike the TCP client).
 *
 * Only the event backend and the per-mode run_* loops are split behind
 * #ifdef _WIN32; timing wrappers, RSS sampling, JSON emission, argument
 * parsing and dispatch are shared.
 * ========================================================================== */
