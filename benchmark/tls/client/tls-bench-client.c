...
static int         g_target_conns  = 10000;
...
static void update_epoll(int fd, uint32_t events) {
...
static int do_handshake(conn_t* c) {
    ...
    if (err == SSL_ERROR_WANT_READ) {
        update_epoll(c->fd, EPOLLIN);
    } else if (err == SSL_ERROR_WANT_WRITE) {
        update_epoll(c->fd, EPOLLOUT);
    } else {
        return -1;
    }
    return 1;
...
static int create_connection(struct sockaddr_in* sa) {
    ...
    g_conns[fd].fd        = fd;
    g_conns[fd].ssl       = ssl;
    g_conns[fd].state     = STATE_CONNECTING;
    g_conns[fd].awaiting_echo = false;
    g_total_created++;
    return fd;
}
...
int main(int argc, char** argv) {
    ...
    int max_fds = g_target_conns + 100;
    g_conns = calloc((size_t)max_fds, sizeof(conn_t));
    ...
    while (created < g_target_conns) {
        ...
        for (int i = 0; i < to_create; i++) {
            if (create_connection(&sa) < 0) break;
            created++;
        }
        ...
        for (int i = 0; i < nev; i++) {
            conn_t* c = &g_conns[events[i].data.fd];
            if (c->state == STATE_CONNECTING && (events[i].events & EPOLLOUT)) {
                int err = 0;
    ...
    for (int fd = 0; fd < max_fds; fd++) {
        if (g_conns[fd].state == STATE_READY) {
            send_echo(&g_conns[fd]);
        }
    }
    ...
    for (int fd = 0; fd < max_fds; fd++) {
        if (g_conns[fd].ssl) {
            SSL_shutdown(g_conns[fd].ssl);
            SSL_free(g_conns[fd].ssl);
        }
        if (g_conns[fd].fd > 0) close(g_conns[fd].fd);
    }
    close(g_epfd);
    free(g_conns);
    free(g_latencies);
